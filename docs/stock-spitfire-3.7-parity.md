...
Current source records 10 VERIFIED, 5 IMPLEMENTED, 5 PARTIAL, and 5 NOT STARTED
...
| B-011 | File tagging and batch queues | SF37 §11.2, §11.4–11.5 | IMPLEMENTED | Schema 16 and the Files session provide a bounded session-ephemeral stable-FileId queue, whole-batch reservation, per-item reauthorization/settlement, localized totals, stale detection, and batch-only protocol enforcement. Caller-visible member-level partial/recompute/continue/skip/cancel and complete live multinode queue acceptance remain before VERIFIED. |
...
| B-014 | File ratios, daily limits, no-charge areas, preview areas, upload time credit | SF37 §3.2, §4.2, §11 | IMPLEMENTED | Schema 16 implements versioned DLPD/decimal-KB/VWR/VER policy, atomic board-day reservations, ratio enforcement/restoration, no-charge counter exclusions, Preview denial before negotiation, and capped idempotent completed-upload time credit. Complete live DST/no-DST, boundary, concurrent-credit, and caller-facing policy acceptance remains. |
...
| B-023 | CD-ROM/extended download-directory behavior | SF37 §4.2, §5.6 | IMPLEMENTED | Schema 16 implements stable versioned roots/locators, confined managed/external resolution, read-only mutation denial, runtime availability distinct from Missing, active-use conflicts, versioned rebind/probe, and bounded large-source streaming. Media loss, restored-board rebind/probe, explicit FA mapping, `SFFILES.<x>`, and complete read-only client acceptance remain. |
