...
Current source records 14 VERIFIED, 2 IMPLEMENTED, 4 PARTIAL, and 5 NOT STARTED
...
| B-011 | File tagging and batch queues | SF37 §11.2, §11.4–11.5 | VERIFIED | Schema 16 and the Files session provide a bounded session-ephemeral stable-FileId queue, whole-batch reservation, per-item reauthorization/settlement, localized list/remove/recompute/clear and chargeable totals, and batch-only protocol enforcement. Member-aware send results settle/remove only completed members, release failed/unstarted reservation authority, preserve remaining order, and recompute changed items before transfer. Session lifecycle, stale state, partial results, multinode quota/accounting, and real Qodem/SyncTERM batch integration pass. |
...
| B-014 | File ratios, daily limits, no-charge areas, preview areas, upload time credit | SF37 §3.2, §4.2, §11 | VERIFIED | Schema 16 implements versioned DLPD/decimal-KB/VWR/VER policy, atomic board-day reservations, reasoned ratio warning/enforcement/restoration, exact no-charge counter exclusions, Preview denial before negotiation, and capped idempotent completed-upload credit. Exact quota/ratio boundaries, spring/fall DST, non-DST midnight, timezone versioning, cross-midnight settlement, multinode contention, Active/PendingReview credit, retries/caps, migration/recovery, privacy, and shared client journeys pass. Exact stock rounding/reset/prompt trivia remains separately evidence-gated. |
...
| B-017 | Sysop logs, daily statistics, notifications, maintenance views | SF37 §8.2, §12 | VERIFIED | Schema 18 provides privacy-bounded structured events, board-day summaries, versioned retention and bounded cleanup, actionable notifications, maintenance/status/recent-caller/message/file/transfer projections, capability checks, a bounded live ring with explicit gap recovery, localization, and cold recovery. Multinode ordering/totals, timezone boundaries, privacy, stable paging, concurrency, restart, backup/restore, and real caller journeys pass. Exact CALLERS.LOG/SFLOG bytes remain separately evidence-gated; general controls and publication remain B-021/B-022. |
...
| B-021 | Local/Sysop operator controls | SF37 §8, §12–13 | PARTIAL | Cross-platform B021-A attachment remains accepted. Schema-19 B021-B1/B2/B3 and integrated B021-B are COMPLETE / ACCEPTED: authenticated minor-gated discovery, exact-session CommandId/receipt recovery, read-only six-capability bootstrap, explicit bounded 32-capability enrollment, durable audit, acknowledgement, signed session time, page availability/answer/decline, consent-based ephemeral chat with time pause, graceful notice/no-notice disconnect, and daemon-only shutdown. sfmonitor provides confirmed capability-aware Actions; Q quits only the monitor. Native macOS integrated acceptance passed; live Windows B021-B remains deferred. [Integrated report](research/m039-tranche-7-b021b3-shutdown-integrated.md). B021-C typed configuration / sfconfig is COMPLETE / ACCEPTED: daemon-only online authority, explicit locked offline authority, revision/digest CAS, atomic recoverable saves, bounded explicit profiles, secret-safe projection, and real sfmonitor handoff pass native macOS acceptance. [Configuration report](research/m039-tranche-7-b021c-sfconfig.md). B021-D remains; this parity row is not VERIFIED. |
| B-022 | Screen/export/print-oriented operations | SF37 §10, §12 | NOT STARTED | Preserve export/report outcomes; printers are optional destinations, not core dependencies. The Tranche 7 gate defines confined screen/text/BBS/CLR/structured output, but implementation remains absent. |
| B-023 | CD-ROM/extended download-directory behavior | SF37 §4.2, §5.6 | VERIFIED | Schema 16 implements stable versioned roots/locators, confined managed/external resolution, read-only mutation denial, runtime availability distinct from Missing, active-use conflicts, versioned rebind/probe, and bounded staging. Generated large-source/media-loss, managed-only backup with external catalog preservation, absent-media restore/rebind, explicit ordered FA mapping, and semantic `SFFILES.<x>` publication pass. Raw DOS paths and exact undocumented adapter bytes never become native authority. |
| B-024 | Built-in ASCII, XMODEM variants, YMODEM variants, ZMODEM, and Telink transfer protocols | SF37 §11.3–11.5 | VERIFIED | All nine required choices pass schema-16 preflight/reservation/settlement and bounded failure tests. Independent original Qmodem passes both `-g` modes in both directions; independent lrzsz passes a real three-member ZMODEM upload; original BinkleyTerm proves bidirectional TeLink. Current-schema SyncTERM/Qodem X/Y/Z and real OpenSSH-carried XMODEM complete the client/carrier matrix. Schema 17 makes zero-byte files valid native authority through every required engine. Exact stock ASCII newline/prompt timing remains separately evidence-gated. |
