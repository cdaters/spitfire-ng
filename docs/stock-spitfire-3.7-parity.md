...
| B-021 | Local/Sysop operator controls | SF37 §8, §12–13 | PARTIAL | `spitfire console` provides status, page availability/answer/decline/chat, targeted disconnect, caller list, enable/disable, security change, and clean exit. Time grants, richer maintenance, attachable control IPC, and complete stock command breadth remain. The Tranche 7 gate fixes the future typed capability/attach boundary; no new general control was added by B-017. |
