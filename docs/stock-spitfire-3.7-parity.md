...
| B-021 | Local/Sysop operator controls | SF37 §8, §12–13 | PARTIAL | Existing `spitfire console` actions remain. Cross-platform B021-A adds schema-19 bounded command receipts/control audit, protected Unix peer-UID and Windows named-pipe peer-SID attachment, protocol/capability negotiation, daemon generation, B-017 read projection transport, bounded live-event reads, and localized noninteractive `spitfire operator` commands through shared `OperatorService`. The read-only `sfmonitor` 0.1 MVP now provides Dashboard, Nodes, Callers, Activity, Statistics, Notifications, Maintenance / Errors, contextual help, reconnect, and the honest future System Configuration doorway over that same client. IPC remains strictly read-only. Stale-safe live actions/time/shutdown, typed configuration/`sfconfig`, and integrated acceptance remain. |
