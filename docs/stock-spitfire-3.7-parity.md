...
Current source records 15 VERIFIED, 2 IMPLEMENTED, 3 PARTIAL, and 5 NOT STARTED
...
| B-021 | Local/Sysop operator controls | SF37 §8, §12–13 | VERIFIED | B021-A/B/C remain accepted; B021-D completes maintenance-owner guidance, coherent setup/offline/restore recovery, capability reconciliation, terminal/result visibility, and native integrated acceptance. Protected daemon authority, read-only bootstrap, 16 explicit capabilities within the unchanged 32-entry ceiling, stale targets, CommandId receipts/audit, page/chat, ±5-minute adjustment, notice/no-notice disconnect, daemon shutdown, sfmonitor → sfconfig handoff, configuration CAS/effects, and cold recovery pass. Existing caller/security and stopped-board domain editors retain their accepted owner routes; no generic maintenance executor is added. Chat is not persisted and secrets are not projected. Real Apple Silicon macOS acceptance and full regression gates pass; Windows interactive confirmation remains deferred under the accepted platform policy, not a missing semantic implementation. See the [B021-D closure and historical outcome mapping](research/m039-tranche-7-b021d-operator-closure.md). B-022 remains NOT STARTED. |
