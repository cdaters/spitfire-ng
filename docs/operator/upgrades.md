...
evidence. Current source migrations exist through schema 11. Native restore
accepts exact schema-10 and schema-11 snapshots but does not convert them:
schema 10 migrates only on later normal writable startup.
...
5. Install/build the proposed new checkpoint.
...
Schema-10 binaries reject schema-11 boards/backups. Use the old executable plus
the pre-upgrade schema-10 backup to roll back; do not edit a schema-11 manifest
or database to impersonate schema 10. Unsupported older/newer snapshots are an
intentional fail-safe boundary.
...
presentation and language packages are board-local too. M038 does not include
