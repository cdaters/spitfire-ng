...
//! Bounded full-duplex BinkP 1.0 session over an owned TCP stream.
...
fn advertise(wire: &mut Wire, plan: &Plan) -> Result<(), Error> {
    ...
    wire.send(Command::Nul, "VER SPITFIRE-NG/0.1 binkp/1.0")
}
                    ...
                    if !authenticated {
                        ...
                        match command {
                            Command::File => {
                                ...
                                let offer = Offer::parse(args, true)?;
                            ...
                            Command::Got | Command::Skip | Command::Get => {
                                let offer = Offer::parse(args, command == Command::Get)?;
        ...
        if authenticated && wire.output.is_empty() {
            ...
            if sending.is_none() {
                if let Some(item) = pending.pop_front() {
                    ...
                    wire.send(Command::File, &item.offer.arguments(true))?;
