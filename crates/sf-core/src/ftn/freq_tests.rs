...
fn freq_schema_26_migration_preserves_acknowledged_request_and_is_atomic() {
    ...
    let id = request(&mut b, &["WANT.TXT"]);
    ack(&mut b, NOW);
    // Construct an actual old-schema fixture, without later Events/C2 authorities.
    b.db.connection
        .execute_batch("PRAGMA foreign_keys=OFF;")
        .unwrap();
    // Remove derived Health objects as well when reconstructing schema 26.
    let health: Vec<(String, String)> = b.db.connection
        .prepare("SELECT type,name FROM sqlite_schema WHERE name LIKE 'conference_health_%' AND type IN ('table','trigger','index') ORDER BY type DESC")
        .unwrap().query_map([], |r| Ok((r.get(0)?, r.get(1)?)))
    ...
    b.db.connection
        .execute_batch("DROP TABLE caller_sessions; DELETE FROM schema_migrations WHERE version>=29; PRAGMA foreign_keys=ON;")
        .unwrap();
