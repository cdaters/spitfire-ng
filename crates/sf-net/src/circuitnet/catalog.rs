...
#[serde(deny_unknown_fields)]
pub struct Entry {
    #[serde(default, skip_serializing_if = "Access::is_public")]
    pub access: Access,
    pub id: String,
    pub codename: Codename,
    pub display_name: String,
...
impl Body {
    /// Human fields in; opaque immutable identity generated by the service.
    pub fn add_conference(&mut self, input: ConferenceInput, reuse: bool) -> Result<(), Error> {
        use ring::rand::SecureRandom;
        let occupied = self.entries.iter().any(|e| e.codename == input.codename);
        if occupied != reuse
            || self
                .entries
                .iter()
                .any(|e| e.codename == input.codename && e.status != Lifecycle::Retired)
        {
        ...
        self.entries.push(Entry {
            ...
            historical_reference: String::new(),
            moderator_role: "conference moderator".into(),
        });
        self.entries.sort_by(|a, b| a.id.cmp(&b.id));
        self.schema = 2;
        self.intent = if reuse {
            Intent::Reuse
        } else {
            Intent::Ordinary
        };
    ...
    pub fn select(&self, selector: &str) -> Result<&Entry, Error> {
        if let Some(e) = self.entries.iter().find(|e| e.id == selector) {
            return Ok(e);
        }
        let code = Codename::new(selector).map_err(|_| Error::Invalid)?;
        self.entries
            .iter()
            .filter(|e| e.codename == code)
            .max_by_key(|e| (e.status != Lifecycle::Retired, e.effective_revision))
            .ok_or(Error::Lifecycle)
    }
    pub fn set_lifecycle(&mut self, selector: &str, status: Lifecycle) -> Result<(), Error> {
        let id = self.select(selector)?.id.clone();
        let e = self
            .entries
            .iter_mut()
            .find(|e| e.id == id)
        ...
        if status == Lifecycle::Active {
            self.intent = Intent::Reactivate;
        }
        e.status = status;
        e.effective_revision = self.revision;
        e.retired_revision = (status == Lifecycle::Retired).then_some(self.revision);
        self.validate()
    }
    pub fn validate(&self) -> Result<(), Error> {
        ...
        for e in &self.entries {
            if (self.schema == 1 && e.access != Access::Public)
                || !is_hex(&e.id, 16)
...
impl Signed {
    ...
    pub fn follows(&self, old: Option<&Self>) -> Result<bool, Error> {
        ...
        if self.body.schema < old.body.schema || self.body.published_at < old.body.published_at {
            return Err(Error::Invalid);
        }
            ...
            let action = match (before, e.status) {
                (None, _)
                    if old.is_some_and(|s| {
                        s.body.entries.iter().any(|b| b.codename == e.codename)
                    }) =>
                {
                    "EXPLICITLY REUSED"
                }
...
mod kit_tests {
    ...
    fn official_seed_and_review_have_one_explicit_disposition_per_area() {
        ...
        let previous = Signed::decode(include_bytes!(
            "../../../../docs/circuitnet-ng/config/catalog-history/000001.json"
        ))
        .unwrap();
        previous.verify(&authority).unwrap();
        assert_eq!(previous.follows(None), Ok(true));
        assert_eq!(signed.follows(Some(&previous)), Ok(true));
        for entry in &signed.body.entries {
            assert_eq!(
                entry.access == Access::Sysops,
                ["SUPPORT", "SYSOP", "SPITFIRE", "DOORS"].contains(&entry.codename.as_str())
            );
        }
